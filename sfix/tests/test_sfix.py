...
class SFIX_test(unittest.TestCase):
    ...
    def test_unknown_msg(self):
        ##region
        msg = sfix_unknown()
        expected = "8=SFIX.1\x019=0\x0135=U\x01\x02"
        out = sfix_parse(msg)
    ...
    def test_acknowledge_msg(self):
        ...
        expected = "8=SFIX.1\x019=0\x0135=A\x01\x02"
        out = sfix_parse(msg)
        self.assertEqual(msg, expected)
        self.assertEqual(out[Tag.MESSAGE_TYPE.value].value.decode(), "A")
        ##endregion

    def test_strategy_msg(self):
        ##region
        msg = sfix_strategy("test")
        expected = "8=SFIX.1\x019=8\x0135=T\x0178=test\x01\x02"
        out = sfix_parse(msg)
        self.assertEqual(msg, expected)
        self.assertEqual(out[Tag.MESSAGE_TYPE.value].value.decode(), "T")
        ##endregion

    def test_error_msg(self):
        ##region
        msg = sfix_error(404,"Ressource not found")
        expected = "8=SFIX.1\x019=30\x0135=E\x0136=404\x0137=Ressource not found\x01\x02"
        out = sfix_parse(msg)
        self.assertEqual(msg, expected)
        self.assertEqual(out[Tag.MESSAGE_TYPE.value].value.decode(), "E")
        ##endregion
