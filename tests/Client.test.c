...
void createOrder(SFIX_Pair input[PROPERTY_LEN], char *qty, char *price,
                 char *side) {
  /*#region*/
  input[0].key = SFIX_Tag_Quantity;
  strcpy(input[0].value, qty);
  input[1].key = SFIX_Tag_Price;
  strcpy(input[1].value, price);
  input[2].key = SFIX_Tag_Side;
  strcpy(input[2].value, side);
...
Test(T, strategy, .fini = teardown, .init = setup) {
  /*#region*/
  char msg[SFIX_MSG_SIZE(1)] = {0};
  SFIX_strategy("test",msg);
  char expected[] = "8=SFIX.1\x01"
                    "9=8\x01"
                    "35=T\x01"
                    "78=test\x01\x02";
...
Test(T, error, .fini = teardown, .init = setup) {
  /*#region*/
  char msg[SFIX_MSG_SIZE(2)] = {0};
  SFIX_error(404, "Ressource not found",msg);
  char expected[] = "8=SFIX.1\x01"
                    "9=30\x01"
                    "35=E\x01"
                    "36=404\x01"
                    "37=Ressource not found\x01\x02";
  cr_expect_eq(strcmp(msg, expected), 0);
  /*#endregion*/
}
...
Test(T, toJson, .fini = teardown, .init = setup) {
  /*#region*/
  int size = 1;
  BUFF(size);
  createMessage(size, _list);
  char buffer[1000] = {0};
  SFIX_toJson(_list, true, buffer, 1000);
  puts(buffer);
  /*#endregion*/
}
